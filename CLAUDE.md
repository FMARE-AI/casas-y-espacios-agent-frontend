...
- Cada mensaje **entrante** reinicia la ventana. El evento `message.new` trae `whatsapp_window_expires_at` fresco en la raíz del payload (al lado de `conversation_priority`): mergearlo sobre la conversación en memoria en `ChatPage` y en la bandeja. **Nunca** refetchear la conversación por mensaje — sería un HTTP por cada entrante y el dato ya viaja en el evento. Sin este merge el chat descuenta hasta cero y deshabilita su propio composer sobre una ventana que el cliente acaba de reabrir.
- `useWebSocket` **reconstruye** el payload de `message.new` (`normalizedMsg`) en vez de spreadear el crudo: cualquier campo nuevo a nivel raíz hay que copiarlo explícitamente ahí o nunca llega a los handlers.
- Un `null` en el evento significa que el backend no pudo resolverlo, no que la ventana volvió a ser desconocida: la ventana solo avanza. Conservar el valor cacheado en vez de pisarlo con `null`.
