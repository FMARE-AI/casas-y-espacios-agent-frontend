...
export const ConversationCard = memo(function ConversationCard({
  ...
  const renderTopRight = () => {
    ...
    return <span className="text-[11px] text-text-secondary">{formatWaitTime(elapsed.seconds)}</span>
  }
      ...
      <div className="space-y-2.5">
        <div className="flex justify-between items-start gap-2">
          <div className="flex flex-wrap gap-1.5 min-w-0">
            ...
            {conversation.client?.client_type && conversation.client.client_type !== 'desconocido' && (
              <span className="bg-bg-tertiary text-text-primary text-[9px] px-1.5 py-0.5 rounded font-semibold">{CLIENT_TYPE_LABELS[conversation.client.client_type]}</span>
            )}
            <span className="bg-bg-tertiary text-brand-blue text-[9px] px-1.5 py-0.5 rounded font-extrabold uppercase">{conversation.channel || 'Desconocido'}</span>
            {isAssignedToMe && (
        ...
        <div>
          ...
          {variant === 'B' && conversation.escalation?.reason && (
             <p className="text-xs text-text-secondary mt-1">Motivo: <span className="font-mono text-text-primary">{ESCALATION_REASON_LABELS[conversation.escalation.reason] ?? conversation.escalation.reason}</span></p>
          )}
          {variant !== 'D' && (
