...
const PRIORITY_BADGE_STYLES: Record<
  Extract<ConversationPriority, "alta" | "critica">,
  string
> = {
  alta: "bg-warning text-bg-main",
  critica: "bg-error text-white",
};

const PRIORITY_BADGE_LABELS: Record<
  Extract<ConversationPriority, "alta" | "critica">,
  string
> = {
  alta: "Alta",
...
function getVariant(conversation: Conversation, now: number): CardVariant {
  ...
  if (conversation.status === "escalada") {
    const waitSeconds = conversation.escalation?.escalated_at
      ? Math.floor(
          (now - new Date(conversation.escalation.escalated_at).getTime()) /
            1000,
        )
...
export const ConversationCard = memo(function ConversationCard({
  ...
  const containerStyles = {
    ...
    B: "border-l-[3px] border-l-warning hover:bg-bg-tertiary/40",
    C: "border-l-[3px] border-l-success hover:bg-bg-tertiary/40",
    D: "border-l-[3px] border-l-border-default opacity-70 hover:opacity-90 hover:bg-bg-tertiary/40",
  };

  const statusChipStyles = {
    A: "bg-error/15 text-error",
    A2: "bg-error text-white",
    B: "bg-warning/15 text-warning",
    C: "bg-success/15 text-success",
    D: "bg-border-default text-text-secondary",
  };
  ...
  const elapsed = useMemo(() => {
    ...
    const seconds = timestamp
      ? Math.max(0, Math.floor((now - new Date(timestamp).getTime()) / 1000))
      : 0;
    const minutes = Math.floor(seconds / 60);
    if (minutes < 60) return { value: minutes, unit: "min", seconds };
    const hours = Math.floor(minutes / 60);
    if (hours < 24)
      return { value: hours, unit: hours === 1 ? "hora" : "horas", seconds };
  ...
  const renderTopRight = () => {
    ...
    return (
      <span className="text-[11px] text-text-secondary">
        {formatWaitTime(elapsed.seconds)}
      </span>
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2.5">
        <div className="flex justify-between items-start gap-2">
          <div className="flex flex-wrap gap-1.5 min-w-0">
            <span
              className={`${statusChipStyles[variant]} text-label px-1.5 py-0.5 rounded uppercase`}
            >
              {statusChipText[variant]}
            </span>
            {conversation.client?.client_type &&
              conversation.client.client_type !== "desconocido" && (
                <span className="bg-bg-tertiary text-text-primary text-[9px] px-1.5 py-0.5 rounded font-semibold">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            {!isAdmin && conversation.unread_count > 0 && (
              <span className="inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 bg-error/20 border border-error/40 text-error rounded-full text-[10px] font-bold leading-none">
        ...
        <div>
          <h3 className="text-h3 text-text-primary truncate" title={clientName}>
            {clientName}
          </h3>
          <CaseNumberTag
          ...
          {variant === "B" && conversation.escalation?.reason && (
            <p className="text-xs text-text-secondary mt-1">
              ...
              <span className="font-mono text-text-primary">
                {ESCALATION_REASON_LABELS[conversation.escalation.reason] ??
                  conversation.escalation.reason}
              </span>
            </p>
          )}
      ...
      <div className="mt-3 pt-2.5 border-t border-border-default/60 flex items-center justify-between">
        {(variant === "A" || variant === "A2") && (
          <>
            <span className="text-error text-[11px] font-bold flex items-center gap-1">
              <svg
            ...
            {isAdmin ? (
              <button
                ...
                className="bg-transparent hover:bg-bg-tertiary border border-border-default text-text-primary px-2.5 py-1 rounded-control text-[11px] font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-blue/90"
              >
                Ver
              </button>
            ) : variant === "A" ? (
              <button
                ...
                className="bg-brand-blue hover:bg-brand-blue-hover text-white px-3 py-1 rounded-control text-[11px] font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-text-primary/90"
              >
                Atender ya
              </button>
            ) : (
              <div className="relative group">
                ...
                {isAtLimit && (
                  <div className="absolute bottom-full right-0 mb-1 hidden group-hover:block bg-bg-main border border-border-default text-text-primary text-[11px] px-2.5 py-1.5 rounded whitespace-nowrap z-50">
                    Límite alcanzado — tienes {advisorActiveConversations}{" "}
        ...
        {variant === "B" && (
          <>
            <div className="flex items-center space-x-1.5 overflow-hidden max-w-[70%]">
              <div className="w-[18px] h-[18px] rounded-full bg-bg-tertiary text-[9px] font-bold flex items-center justify-center text-white">
                {(
                  conversation.escalation?.advisor?.full_name?.[0] || "A"
                ).toUpperCase()}
              </div>
              <span className="text-text-secondary text-[11px] truncate">
                Asignado:{" "}
                <strong className="text-white font-semibold">
                  {conversation.escalation?.advisor?.full_name?.split(" ")[0] ||
                    "Asesor"}
                </strong>
              </span>
            </div>
        ...
        {variant === "C" && (
          <>
            <span className="text-text-secondary flex items-center gap-1.5 text-[11px] truncate max-w-[70%]">
        ...
        {variant === "D" && (
          <>
            ...
            <button
              onClick={() => onView(conversation.id)}
              className="bg-transparent hover:bg-bg-tertiary border border-border-default text-text-secondary hover:text-text-primary px-2.5 py-1 rounded-control text-[11px] font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-blue/90"
            >
              Ver
            </button>
          </>
