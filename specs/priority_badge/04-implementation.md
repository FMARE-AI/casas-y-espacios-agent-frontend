...
### Task 6 — Pulse animation for `critica`
Status: ✅ Done
Notes: User asked for the badge to visibly pulse when priority is `critica` (not `alta`). Reused the existing `criticalPulse` `@keyframes` in `src/index.css` (already used by `.critical-pulse-card` for the wait-time-based A2 card variant — a box-shadow halo + border-color pulse in `#FF5B5B`, the same red as the crítica badge). Added a new `.critical-pulse-badge` class with the same animation but without `.critical-pulse-card`'s `border-left: 4px solid #FF5B5B !important` (that's card-specific and would visually collide with the badge's own border). Applied conditionally to the badge span only when `priority === 'critica'` — `alta` stays static. Deliberately did not reuse `.critical-pulse-card` on the whole card itself: that class already carries a distinct meaning (escalation waiting ≥15 min, a time-based signal) and conflating it with `critica` priority (a bot tone/urgency signal) would blur two different things an advisor needs to tell apart.
