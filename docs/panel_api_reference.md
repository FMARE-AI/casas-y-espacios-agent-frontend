...
### Production Flow (Supabase Auth — use this)

The frontend authenticates directly against Supabase Auth, receives a JWT, and passes it as a Bearer token to every FastAPI call. FastAPI only validates the token — it never handles passwords.

```javascript
// 1. Sign in with Supabase SDK
const { data, error } = await supabase.auth.signInWithPassword({
  email: "asesor@casasyespacios.co",
  password: "secretpassword",
});

// 2. Get the JWT from the session
const token = data.session.access_token;

// 3. Use it in every API call
fetch("/api/v1/panel/conversations/", {
  headers: { Authorization: `Bearer ${token}` },
});
```

The JWT is decoded with `HS256` using `JWT_SECRET`. The `sub` claim must contain the advisor's UUID. The advisor must exist in the `advisors` table and have `is_active = true`.

### Development-only Flow (debug endpoint)

A convenience `POST /api/v1/panel/auth/token` endpoint exists for local development. It issues a JWT **without verifying a password** — only by email lookup. **Do not use in production.**

### POST /api/v1/panel/auth/token

**Auth required:** No

**Description:** Issues a JWT for the given email. Verifies the advisor exists and is active. Does not check password. Development-only convenience endpoint.
...
| HTTP | ErrorCode          | When                                   |
| ---- | ------------------ | -------------------------------------- |
| 401  | `INVALID_TOKEN`    | Email not found in the advisors table  |
...
localStorage.setItem("panel_token", jwt);
...
// 4. Handle token expiry — set up Supabase auth listener
supabase.auth.onAuthStateChange((event, session) => {
  ...
  if (event === "TOKEN_REFRESHED") {
    localStorage.setItem("panel_token", session.access_token);
  }
});
```
...
5. **`POST /api/v1/panel/auth/token` does not validate passwords** — The endpoint accepts only an email in the request body and issues a JWT without any password check. CLAUDE.md describes it as "DEVELOPMENT ONLY" but there is no settings guard (e.g., `DEBUG=true`) to disable it in non-development environments. Recommend adding a `settings.DEBUG` guard before this router is included in `main.py`.

6. **`POST /api/v1/panel/advisors/` does not validate `specialty` against `area`** — The `_validate_specialty_area` helper exists and is used by `PATCH /{advisor_id}`, but it is not called during `POST /`. It is possible to create an advisor with an invalid area/specialty combination.

7. **`behavior.alert` WebSocket event payload contains only `alert_id`** — This is an intentional design choice per the code comment, but it requires the frontend to make a follow-up API call to get the full alert. This is not documented anywhere in CLAUDE.md.

8. **`AdvisorRepository.update_availability` stores `status_until` as a naive Bogotá local time** — The `status_until` value returned from any endpoint echoing the advisor object will be a naive ISO string without a timezone suffix. Frontend code that parses this field must not assume UTC.
